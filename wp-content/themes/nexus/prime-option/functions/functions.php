 ...
 * @return    array
 */
function prime_options_recognized_font_styles()
{
 ...
 * @return    array
 */
function prime_options_recognized_font_weights()
{
 ...
 * @return    array
 */
function prime_options_recognized_font_variants()
{
 ...
 * @return    array
 */
function prime_options_recognized_font_families()
{
 ...
 * @return    array
 */
function prime_options_recognized_background_repeat()
{
 ...
 * @return    array
 */
function prime_options_recognized_background_attachment()
{
 ...
 * @return    array
 */
function prime_options_recognized_background_position()
{
 ...
 * @return    array
 */
function prime_options_measurement_unit_types()
{
 ...
 */
function prime_options_option_tree_css_save()
{
    global $wpdb;

    $options = $wpdb->get_results("SELECT item_id FROM " . OT_TABLE_NAME . " WHERE `item_type` = 'css' ORDER BY `item_sort` ASC");
    foreach ($options as $option)
        prime_options_option_tree_insert_css_with_markers($option->item_id);
 ...
 * @return bool True on write success, false on failure.
 */
function prime_options_option_tree_insert_css_with_markers($option = '')
{
    /* No option defined */
    if (!$option)
        return;
    ...
    if (!file_exists($filepath) || is_writeable($filepath)) {
        ...
        $insertion = prime_options_option_tree_normalize_css(stripslashes($options[$option]));
        $regex = "/{{([a-zA-Z0-9\_\-\#\|\=]+)}}/";
        $marker = $option;
            ...
            if (is_array($value)) {
                ...
                if (!isset($option_array[1])) {
                    ...
                    if (isset($value[0]) && isset($value[1])) {
                        ...
                        foreach (prime_options_recognized_font_families() as $key => $v) {
...
function prime_options_option_tree_normalize_css($s)
{
...
function option_tree_render_single_element($value)
{
    ...
    global $po_admin;
    // private page ID
    $post_id = $po_admin->get_option_page_ID('media');

    if ($value->item_type == 'upload' || $value->item_type == 'background' || $value->item_type == 'slider') {
        global $post_id;
        $int = $post_id;
    } else if ($value->item_type == 'textarea' || $value->item_type == 'css' || $value->item_type == 'option_data') {
        $int = (is_numeric(trim($value->item_options))) ? trim($value->item_options)
                : ($value->item_type == 'css' ? 24 : 8);
    }
    elseif ($value->item_type == 'heading') {
        $int = $value->item_options;
    }
    else
    {
        $int = 0;
...
function option_tree_render_single_slider_element($value, $settings)
{
    global $po_admin;
    // private page ID
    $post_id = $po_admin->get_option_page_ID('media');
...
function prime_options_option_tree_hidden($value, $settings, $int)
{
    //add class routing from $value->item_options['class']

    $class = '';
    if (isset($value->item_options) && isset($value->item_options->class)) $class = $value->item_options->class;

    ?>
<input type="hidden" class="<?php echo $class; ?>" name="<?php echo isset($value->item_name) ? $value->item_name : $value->item_id; ?>"
