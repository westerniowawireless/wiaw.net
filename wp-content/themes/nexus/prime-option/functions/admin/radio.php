 ...
 * @return string
 */
function prime_options_option_tree_radio($value, $settings, $int)
{
    ?>
    ...
    <div class="section">
        <div class="element">
            ...
            foreach (explode(',', $value->item_options) as $option)
            {
                ...
                echo '<div class="input_wrap"><input name="' . $item_name . '" id="' . $value->item_id . '_' . $count . '" type="radio" value="' . trim($option) . '"' . $checked . ' /><label for="' . $value->item_id . '_' . $count . '">' . trim($option) . '</label></div>';
                $count++;
            }
            ?>
        </div>
