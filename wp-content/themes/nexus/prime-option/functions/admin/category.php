 ...
 * @return string
 */
function prime_options_option_tree_category($value, $settings, $int)
{
    ?>
 ...
 * @return string
 */
function prime_options_option_tree_categories($value, $settings, $int)
{
    ?>
<div class="option option-checbox">
    ...
    <div class="section">
        <div class="element">
            ...
            // loop through tags
            $categories = &get_categories(array('hide_empty' => false, 'taxonomy' => $value->item_options));
            if ($categories) {
                $count = 0;
                foreach ($categories as $category)
                {
                    ...
                    echo '<div class="input_wrap"><input name="' . $item_name . '[' . $count . ']" id="' . $value->item_id . '_' . $count . '" type="checkbox" value="' . $category->term_id . '"' . $checked . ' /><label for="' . $value->item_id . '_' . $count . '">' . $category->name . '</label></div>';
                    $count++;
                }
            }
            else
