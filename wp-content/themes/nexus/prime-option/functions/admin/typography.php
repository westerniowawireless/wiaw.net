 ...
 * @return string
 */
function prime_options_option_tree_typography($value, $settings, $int)
{
    ?>
<div class="option option-font">
    ...
    <div class="section">
        <div class="element">
            ...
            <input type="text"
                   name="<?php echo isset($value->item_name) ? $value->item_name : $value->item_id; ?>[font-color]"
                   id="<?php echo $value->item_id; ?>-picker"
                   value="<?php echo (isset($settings[$value->item_id]['font-color']))
                           ? stripslashes($settings[$value->item_id]['font-color']) : ''; ?>" class="cp_input"/>

            <div id="cp_<?php echo $value->item_id; ?>" class="cp_box">
                <div style="background-color:<?php echo (isset ($settings[$value->item_id]['font-color']))
                        ? $settings[$value->item_id]['font-color']
                        : '#ffffff'; ?>;<?php if (isset($settings[$value->item_id]['font-color'])) {
            ...
            <div class="select_wrapper typography-family">
                <select name="<?php echo isset($value->item_name) ? $value->item_name
                        : $value->item_id; ?>[font-family]" class="select">
                    <?php
                                echo '<option value="">font-family</option>';
                    foreach (prime_options_recognized_font_families() as $key => $family) {
            ...
            <div class="select_wrapper typography-style" style="width:165px;">
                <select name="<?php echo isset($value->item_name) ? $value->item_name : $value->item_id; ?>[font-style]"
                        class="select">
                    <?php
                                echo '<option value="">font-style</option>';
                        foreach (prime_options_recognized_font_styles() as $key => $style) {
            ...
            <div class="select_wrapper typography-variant" style="width:166px;margin-left:10px;">
                <select name="<?php echo isset($value->item_name) ? $value->item_name
                        : $value->item_id; ?>[font-variant]" class="select">
                    <?php
                                echo '<option value="">font-variant</option>';
                        foreach (prime_options_recognized_font_variants() as $key => $variant) {
            ...
            <div class="select_wrapper typography-weight" style="width:165px;">
                <select name="<?php echo isset($value->item_name) ? $value->item_name
                        : $value->item_id; ?>[font-weight]" class="select">
                    <?php
                                echo '<option value="">font-weight</option>';
                        foreach (prime_options_recognized_font_weights() as $key => $weight) {
            ...
            <div class="select_wrapper typography-size" style="width:166px;margin-left:10px;">
                <select name="<?php echo isset($value->item_name) ? $value->item_name : $value->item_id; ?>[font-size]"
                        class="select">
                    <?php
