 ...
 */
function get_prime_options( $item_id = '', $options = '', $echo = false, $is_array = false, $offset = -1) {
  // load saved options
  if ( !$options )
  ...
  $content = prime_options_option_tree_stripslashes( $options[$item_id] );
 ...
 * @return mixed
 */
function prime_options_option_tree_stripslashes( $input ) {
  if ( is_array( $input ) ) {
    foreach( $input as &$val ) {
      if ( is_array( $val ) ) {
        $val = prime_options_option_tree_stripslashes( $val );
      } else {
