...
<div id="framework_wrap" class="wrap">
  ...
  'offset'    => -1,
          ...
          <h3>Examples</h3>
          <p>
            This example assigns the value of the <code>get_option(PRIME_OPTIONS_KEY)</code> array to the variable <code>$theme_options</code> for use in PHP. You would then add this code to the top of your header.php file and use that variable as the <code>$options</code> variable in the <code>get_option_tree()</code> function. This helps to reduce database queries anytime you want to request multiple theme options in a template files. This is optional, but may help speed up things up.
            <pre><code>&lt?php $theme_options = get_option(PRIME_OPTIONS_KEY); ?&gt;</code></pre>
          </p>
