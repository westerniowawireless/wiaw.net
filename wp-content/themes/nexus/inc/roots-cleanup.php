...
class roots_nav_walker extends Walker_Nav_Menu
{
    function start_el(&$output, $item, $depth, $args)
    {
        ...
        $item_output .= '<a' . $attributes . '>' . '<span class="menu-link-wrap">';
...
function roots_notice_tagline()
{
    ...
    if (!get_user_meta($user_id, 'ignore_tagline_notice')) {
        ...
        echo '<p>', sprintf($FRONTEND_STRINGS['update_site_tagline'], admin_url('options-general.php'), '?tagline_notice_ignore=0'), '</p>';
        echo '</div>';
    }
}
